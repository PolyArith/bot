...
void updateLibrary(string answer, vector<string> wordList){
    if (find(wordList.begin(), wordList.end(), answer) == wordList.end()){
        ofstream outFile("wordLibrary.txt", std::ios_base::app);
        outFile << endl << answer;
    }
}